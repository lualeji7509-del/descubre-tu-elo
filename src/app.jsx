...
function Board({ game, orientation, selected, targets, lastMove, flash, onSquare, disabled, decorative = false }) {
 ...
 return (
  ...
  (flash === 'ok' ? 'ring-4 ring-emerald-400' : flash === 'bad' ? 'ring-4 ring-ember animate-shake' : 'ring-line')
  }
  >
  {ranks.map((rank) =>
  files.map((file) => {
...
function AutoBoard({ fen, moves }) {
 ...
 return (
  <Board game={gRef.current} orientation="w" selected={null} targets={[]}
  lastMove={lastRef.current} flash={null} onSquare={() => {}} disabled decorative />
  );
}
...
function App() {
 ...
 return (
  ...
  <a href="#maestros" className="hover:text-gold transition">{t.mastersTitle}</a>
  <a href="#reto" className="hover:text-gold transition">{t.quizTitle}</a>
  <a href="#historia" className="hover:text-gold transition">{t.navHistory}</a>
  </nav>
  <div className="inline-flex overflow-hidden ring-1 ring-line text-[11px] font-bold">
  {['es', 'en'].map((l) => (
  <button key={l} onClick={() => setLang(l)} aria-pressed={lang === l}
  className={'px-2.5 py-1 transition ' + (lang === l ? 'bg-gold text-ink' : 'bg-white/5 text-ivory/60')}>
  {l.toUpperCase()}
  </button>
  ))}
  </div>
  ...
  {masters.map((m) => (
  <article key={m.key} className="reveal group relative bg-panel ring-1 ring-line p-6 flex flex-col overflow-hidden hover:ring-gold/40 transition">
  {/* la pieza, enorme y apagada, como marca de agua */}
  <Piece type={m.piece} color="w"
  className="absolute -right-6 -bottom-6 w-40 h-40 opacity-[.05] group-hover:opacity-[.09] transition" />

  <p className={'t-label ' + m.accent}>{m.title} · {m.role}</p>
  <h3 className="display text-2xl font-black leading-tight mt-2">{m.name}</h3>

  <div className="mt-5 flex items-baseline gap-3">
  <span className="display text-5xl sm:text-6xl font-black tabular-nums text-gold leading-none">{m.elo}</span>
  ...
  <p className="text-center text-[11px] uppercase tracking-[0.25em] text-ivory/50">{t.pickLevel}</p>
  {[
  ...
  <p data-elo className="display t-huge font-black text-gold tabular-nums mt-1">{elo}</p>
  ...
  {t.history.map((h, i) => (
  <li key={i} className="reveal bg-ink p-5 sm:p-7 grid sm:grid-cols-[8rem_1fr] gap-2 sm:gap-7 hover:bg-panel transition">
  <p className="display text-gold text-2xl sm:text-3xl font-black tabular-nums leading-none">{h.era}</p>
  <div>
  ...
  className="text-sm font-bold text-ivory/70 hover:text-gold transition">@{m.ig}</a>
  ))}
  </div>
